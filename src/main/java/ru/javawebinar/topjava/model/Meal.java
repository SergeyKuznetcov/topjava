...
@NamedQueries({
        ...
        @NamedQuery(name = Meal.UPDATE, query = "UPDATE Meal m SET m.dateTime=?1, m.description=?2, m.calories=?3 " +
                "WHERE m.id=?4 AND m.user.id=?5"),
})
@Entity
@Table(name = "meals", uniqueConstraints = {@UniqueConstraint(columnNames = {"user_id", "date_time"}, name = "uk_user_date_time")})
